...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # Initiate a string that is contents + additional contents
        # Reset self.contents (data will be stored in above variable
        # Loop over the length of the contents + additional contents.
        # If the index is less than the volume - 1 then add the characters
        #   in the summed string to self.contents
        # Else add the rest to the variable returning the appended string

        total_contents = self.contents + additional_contents
        self.contents = ''
        for k in range(len(total_contents)):
            if k <= self.volume - 1:
                self.contents += total_contents[k]
            else:
                self.appended_contents += total_contents[k]
        return self.appended_contents
    ...
    def shrink(self, new_volume):
        ...
        # set the new volume
        # append self with no additional characters
    ...
    def double_then_shrink(self, new_volume):
        ...
        discarded = 0
        self.double()
        discarded += len(self.shrink(new_volume))
        return discarded
